...
<div id="container">
    ...
    <div id="wrapper">
        <div id="content">
            ...
            <c:if test="${authUser ne null and !empty map.post}">
                <form action="${pageContext.request.contextPath }/cmtpush" method="post">
                        ...
                        <ul class="blog-list">
                            <c:if test="${!empty map.post}">
                                <c:forEach items="${map.post}" var="i" begin="0" end="${map.postCount}">
                                    <li>
                                        <a href="${pageContext.request.contextPath }/${map.blog.id}/mainPage?crtPage=${map.crtPage}&cateNo=${i.cateNo}&postNo=${i.postNo}">${i.postTitle}</a>
                                        <span>${i.regdate}</span>
                                    </li>
                                </c:forEach>
                            </c:if>
                ...
                <tr>
                    <td align="center">
                        ...
                        <c:if test="${map.prev eq true}">
                            <a href="${pageContext.request.contextPath }/${map.blog.id}/mainPage/?crtPage=${map.startPageBtnNo-1}&cateNo=${map.getPostOne.cateNo}&postNo=${map.getPostOne.postNo}">◀</a>
                        </c:if>

                        <%--반복문 시작--%>
                        <c:forEach var="pageNo" begin="${map.startPageBtnNo}" end="${map.endPageBtnNo}">
                            <a href="${pageContext.request.contextPath }/${map.blog.id}/mainPage/?crtPage=${pageNo}&cateNo=${map.getPostOne.cateNo}&postNo=${map.getPostOne.postNo}">${pageNo}</a>
                            <%--반복문 끝--%>
                        </c:forEach>

                        <%--if문 처리--%>
                            <c:if test="${map.next eq true}">
                        <a href="${pageContext.request.contextPath }/${map.blog.id}/mainPage/?crtPage=${map.endPageBtnNo+1}&cateNo=${map.getPostOne.cateNo}&postNo=${map.getPostOne.postNo}">▶</a>
                            </c:if>
                    </td>
                </tr>
            </table>
    ...
    <div id="extra">
        <div class="blog-logo">
            ...
            <img src="${pageContext.request.contextPath }/upload/${map.blog.logoFile}">
            </c:if>
        </div>
        <div id="navigation">
            <h2>카테고리</h2>
            <ul>
                <c:forEach items="${map.category}" var="i">
                    <c:if test="${i.cateNo eq cateNo}">
                        <li><strong><a href="${pageContext.request.contextPath }/${map.blog.id}/mainPage?cateNo=${i.cateNo}&postNo=0"
                                       style="font-size: large">${i.cateName}</a></strong></li>
                    </c:if>
                    <c:if test="${i.cateNo ne cateNo}">
                        <li><a href="${pageContext.request.contextPath }/${map.blog.id}/mainPage?cateNo=${i.cateNo}&postNo=0">${i.cateName}</a></li>
                    </c:if>
                </c:forEach>
            </ul>
        </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        console.log(authId);
        $.ajax({
            url: "${pageContext.request.contextPath }/getCmt",
            type: "post",
            data: {postNo: postNo},
    ...
    function rander1(cmt) {
        ...
        str += "    <td align='center' style='border: 1px black solid; width:5%; padding:5px;'><img src='${pageContext.request.contextPath }/assets/images/delete.jpg' id='" + cmt.cmtNo + "'></td>"
        str += "</tr>"
    ...
    $("[name=cmtPush]").on("click", function () {
        ...
        console.log(postNo);
        $.ajax({
            url: "${pageContext.request.contextPath }/addcmt",
            type: "post",
    ...
    $("#cmt_list").on("click", "img", function () {
        ...
        console.log($this);
        $.ajax({
            url: "${pageContext.request.contextPath }/delcmt",
            type: "post",
            data: {cmtNo: $this},
